...
// PRIMER VIDEO A MOSTRAR
const obtenerVideo = (array) => {
    return new Promise((res, rej)=>{
        res(array[$numeroPagina.innerText])
    })
}
let video = clases[$numeroPagina.innerText]
...
$btnSiguiente.addEventListener('click', () =>{
    if($numeroPagina.innerText < clases.length){
        ...
        mostrarContenido(parseInt($numeroPagina.innerText));
        obtenerVideo(clases)
            .then((valor)=> {
                video = valor;
            })
